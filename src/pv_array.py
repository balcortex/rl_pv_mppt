...
class PVArray:
    ...
    def _simulate_cached(self, v: float, g: float, t: float) -> PVSimResult:
        logging.debug(f"Simulate called with args: V={v}, G={g}, T={t}")
    ...
    def get_true_mpp(
        ...
        logging.debug("Getting true MPP . . .")
        ...
        for (idx), (g, t) in enumerate(zip(irradiance, temperature)):
            logging.debug(f"Idx: {idx}/{len(irradiance) - 1}")
    ...
    def mppt_po(
        ...
        logging.debug(f"Running P&O, delta_v={v_eps} . . .")
        voltages = []
        currents = []
        powers = []
        p0 = 0
